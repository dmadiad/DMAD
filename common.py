...
class FeatureFusionModule(torch.nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.conv = torch.nn.Conv2d(input_dim, output_dim, kernel_size=(1,1))
...
class NetworkFeatureAggregator(torch.nn.Module):
    ...
    def __init__(self, backbone, layers_to_extract_from, device, train_backbone=False, backbone_type="resnet"):
        ...
        for extract_layer in layers_to_extract_from:
            if self.backbone_type == "resnet":  # restnet backbone
                ...
                if "." in extract_layer:
                    ...
                    # get the submodule of the backbone, the '__dict__' can get the attribute of the backbone
                    network_layer = backbone.__dict__["_modules"][extract_layer]
            else: # vits backbone
                forward_hook = ForwardHook(
                    self.outputs, "layer{}".format(extract_layer), "layer{}".format(layers_to_extract_from[-1])
                )
                network_layer = backbone.__dict__["_modules"]["blocks"][extract_layer]
    ...
    def feature_dimensions(self, input_shape):
        ...
        if self.backbone_type == "resnet":
            ...
            return [_output["layer{}".format(layer)].shape[1] for layer in self.layers_to_extract_from]

class ForwardHook:
    def __init__(self, hook_dict, layer_name: str, last_layer_to_extract: str):
        self.hook_dict = hook_dict
        self.layer_name = layer_name
        self.raise_exception_to_break = \
